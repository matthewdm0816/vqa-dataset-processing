...
logging.info("Filling Empty Image Features")


def fill(samples):
    slen = len(samples["question_id"])
    return {
        "feature": [
            [
                [
        ...
        "tags": ["" for _ in range(slen)],
    }


sorted_dataset = sorted_dataset.map(fill, batched=True)
...
sorted_dataset = sorted_dataset.map(feature_loader.batched_load, batched=True)
...
sorted_dataset.save_to_disk("processed_with_features")
